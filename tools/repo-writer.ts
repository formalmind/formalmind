...
export class TemplateRepoWriterAgent {
	...
	async writeAndPushFiles(
		...
		// 👇 Set modelingDir early so it's available for all methods
		await fs.mkdir(this.modelingDir, { recursive: true });

		// Write modeling files (Lean + JSON + Namespace + Imports)
		...
		// Update the modeling index with new entry
		await this.updateModelingIndex({
			commitSha: this.meta?.commitSha ?? "",
			json,
		});

		// ✅ Regenerate Main.lean with updated imports/opens
		await this.writeMainLean();

		const repoGit = simpleGit({ baseDir: this.tmpDir });

		// 🛠️ Safety commit before pull (unstaged -> staged)
		...
		try {
			await repoGit.pull('origin', defaultBranch, { '--rebase': null });
		} catch (err) {
			console.error("⚠️ Git pull with rebase failed. Continuing with push...", err);
		}
	...
	async run() {
		...
		if (!exists) {
			...
			// reconstruct clone URL and assume default branch is `main` unless otherwise
		...
		await this.writeAndPushFiles(cloneUrl, defaultBranch, lean, json);

		console.log("✅ Repo content added and pushed.");
	}
}
